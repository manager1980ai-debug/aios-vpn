...
bool Xray::startXray(const QString &cfg)
{
    ...
    if (auto err = aios_xray_setsockcallback(ctxSockCallback, this); err != nullptr) {
        qDebug() << "[xray] sockopt failed: " << err;
        aios_xray_free(err);
        return false;
    }

    aios_xray_setloghandler(ctxLogHandler, this);

    QByteArray bytes = cfg.toUtf8();
    if (auto err = aios_xray_configure(bytes.data()); err != nullptr) {
        qDebug() << "[xray] configuration failed: " << err;
        aios_xray_free(err);
        return false;
    }

    if (auto err = aios_xray_start(); err != nullptr) {
...
bool Xray::stopXray()
{
    ...
    bool success = true;
    if (auto err = aios_xray_stop(); err != nullptr) {
